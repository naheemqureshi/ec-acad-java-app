...
sed -i "s#BUILD_NUMBER#$IMAGE_TAG#g" task-definition2.json
sed -i "s#REPOSITORY_URI#$REPOSITORY_URI#g" task-definition2.json
sed -i "s#ROLE_ARN#$ROLE_ARN#g" task-definition2.json
sed -i "s#FAMILY#$FAMILY#g" task-definition2.json
sed -i "s#NAME#$NAME#g" task-definition2.json


aws ecs register-task-definition --cli-input-json file://task-definition2.json --region="${AWS_DEFAULT_REGION}"
